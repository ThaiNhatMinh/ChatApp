...
void Shader::LinkShader()
{
	...
	if (check == GL_FALSE)
	{
		...
		std::cout<< "Program Shader link error -> " + std::string(infoLog);
	}
	...
	for (int i = 0; i < nUniform; i++)
	{
		...
		m_UniformLists.insert({ std::string(name),location });
#if 0
...
//	GLuint vertexShader, fragmentShader;
	...
	if (!pvFile)
	{
		std::cout<< "Can't open File: " + std::string(vertexShaderFile);
		return false;
	}
	...
	if (!pfFile)
	{
		std::cout << "Can't open File: " + std::string(fragmentShaderFile);
		return false;
	}
	...
	if (check == GL_FALSE)
	{
		...
		std::cout << "FILE: " + std::string(vertexShaderFile);
	...
	if (check == GL_FALSE)
	{
		...
		std::cout << "FILE: " + std::string(fragmentShaderFile);

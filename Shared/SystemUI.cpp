...
{
	SystemUI* pw = (SystemUI*)glfwGetWindowUserPointer(w);
	
	if (action == GLFW_PRESS && button >= 0 && button < 3)
		pw->m_MousePress[button] = true;
}
...
void ImGui_ImplGlfwGL3_ScrollCallback(GLFWwindow* w, double /*xoffset*/, double yoffset)
{
	...
	pw->m_MouseWhell += (float)yoffset; // Use fractional mouse wheel.
}
...
SystemUI::SystemUI(Windows* w32):m_Time(0)
{
	...
	m_pShader = std::unique_ptr<Shader>(new Shader("Data\\Shader\\imgui.vs", "Data\\Shader\\imgui.fs"));
	m_Mesh = std::make_unique<imguiMesh>();
	m_Mesh->Finalize(m_pShader.get());
